...
# this first function comme from : http://community.spiceworks.com/scripts/show/1887-get-telnet-telnet-to-a-device-and-issue-commands
<#
.SYNOPSIS
...
function Convert-UrlEncode($val){
    [System.Web.HttpUtility]::UrlEncode($val)
}


function ConvertTo-URL($hosturl, $queryParams){
    
    $queryarr = @()
    $queryParams.GetEnumerator() | ForEach-Object {
        $name = Convert-UrlEncode $_.name
        $value = Convert-UrlEncode $_.value
        $queryarr += "$name=$value"
    }
    
    $querystr = $queryarr -join "&"

    if("$querystr" -eq ""){ return $hosturl }
    return "$hosturl`?$querystr"

}


function Get-FileName($url, $default){
...
function Get-File($url, $dest_path){
